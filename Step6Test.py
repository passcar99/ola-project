...
if __name__ == '__main__':
    ...
    idx_max_phase3=np.argmax(Clayrvoiant[:,2])
    ...
    clayt_opt3=[idx_max_phase3%n_arms,idx_max_phase3%pow(n_arms,2)//n_arms,idx_max_phase3%pow(n_arms,3)//pow(n_arms,2),idx_max_phase3%pow(n_arms,4)//pow(n_arms,3),idx_max_phase3//pow(n_arms,4)]
    print(opt)
    print("Optimum phase 1: ")
    print(clayt_opt1)
    print("Optimum phase 2: ")
    print(clayt_opt2)
    print("Optimum phase 3: ")
    print(clayt_opt3)
    ...
    for e in tqdm(range(n_experiments)):
        ...
        ex_ucb_sliding_rewards_per_experiment.append(ex_reward_ucb_sliding)
    ...
    for brk_idx in range(len(breakpoints)-1):
        ex_reward_Clayr[breakpoints[brk_idx]:breakpoints[brk_idx+1]]=opt[brk_idx+1]
    ex_reward_Clayr[breakpoints[-1]:]=opt[-1]

    #REWARD COMPARISON PLOT TO SAVE
    fig = plt.figure(1)
    plt.ylabel("Expected reward")
    plt.xlabel("t")
    #plt.plot(np.arange(0, T), np.mean(ex_ucb_rewards_per_experiment, axis = 0), 'r')
    ...
    #EX_ IN FRONT TO HAVE THE PSEUDO REWARD
    plot_and_save_rewards([#ex_ucb_rewards_per_experiment,
                        ucb_detecting_rewards_per_experiment,ucb_sliding_rewards_per_experiment],
                        ex_reward_Clayr, [#"UCB vanilla", 
                        "UCB detecting", "UCB sliding",], EXPERIMENT_NAME, T, display_figure=DISPLAY_FIGURE)
